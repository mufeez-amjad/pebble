...
func (d *DB) manualCompact(start, end InternalKey, level int, parallelize bool) error {
	...
	files := curr.Overlaps(level, d.cmp, start.UserKey, end.UserKey, end.IsExclusiveSentinel())
	if files.Empty() {
		d.mu.Unlock()
		return nil
	}

	splitCompactions := d.splitManualCompaction(start.UserKey, end.UserKey, level, parallelize)
	...
	// compaction is dropped, or executes with/without an error after being
	// scheduled. Both outcomes send a value to the done channel. Since the
	// channels are buffered, it is not necessary to read from each channel, and
	// so we can exit early in the event of an error.
...
func (d *DB) splitManualCompaction(
	start, end []byte, level int, parallelize bool,
